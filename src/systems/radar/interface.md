...
当 [雷达模式旋钮](#雷达模式mode旋钮) 转至 MAP 时，自动指令进行 1 BAR 扫描。对于 BST、AIR-GND 和
CAGE 状态，扫描开关档位无功能。
...
[MAN Vc](#手动接近率man-vc) 旋钮将用于设置跟踪门接近率。
...
在 [AIR-GRD](#air-grd空对地档位) 和 [计算机自动截获](operation.md#计算机自动截获模式caa) 模式下，系
